...
const mod = await req(
  'POST',
  '/modules',
  { number: modNum, title: { de: 'Bewertungs-Test' } },
  teacher,
);
...
check(
  'Queue enthält Einreichung',
  Array.isArray(queue.body) && queue.body.some((x) => x.id === submissionId),
);
...
const reject = await req(
  ...
  { reason: 'Bitte Topologie ergänzen.' },
  teacher,
);
...
check(
  'Lernende:r sieht rejected + Begründung',
...
const sub2 = await req(
  ...
  { text: 'Doku v2 mit Topologie.' },
  student,
);
const submissionId2 = sub2.body?.submissionId;
check(
  'Erneute Einreichung möglich',
  sub2.body?.status === 'SUBMITTED' && submissionId2 !== submissionId,
);

// ── FA-60: Bewerten ───────────────────────────────────────────────
const tooMany = await req(
  'POST',
  `/submissions/${submissionId2}/evaluation`,
  { points: 99 },
  teacher,
);
check('Punkte > Max → 422', tooMany.status === 422);

const grade = await req(
  'POST',
  `/submissions/${submissionId2}/evaluation`,
  { points: 16, level: 'INTERMEDIATE', feedback: 'Gut, vollständig.' },
  teacher,
);
...
check(
  'Lernende:r sieht graded + Punkte + Feedback',
...
const regrade = await req(
  'POST',
  `/submissions/${submissionId2}/evaluation`,
  { points: 18, level: 'ADVANCED', feedback: 'Nachbesserung top.' },
  teacher,
);
...
check(
  'Historie hat 2 Einträge (created + updated)',
  Array.isArray(hist.body) && hist.body.length === 2,
);
check(
  'Historie chronologisch (neuester zuerst)',
...
check(
  'Rückweisungs-Historie vorhanden',
  hist1.body?.some((h) => h.changeType === 'REJECTED'),
);
