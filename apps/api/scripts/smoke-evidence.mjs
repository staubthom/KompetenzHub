...
const mod = await req(
  'POST',
  '/modules',
  { number: modNum, title: { de: 'Nachweis-Test 2' } },
  teacher,
);
...
check(
  'Einreichungsarten default (file/link/text)',
  ev.body?.config?.allowFile && ev.body?.config?.allowLink && ev.body?.config?.allowText,
);
check(
  'Default: Einfügen gesperrt, Screenshot aus',
...
check(
  'Matrix enthält Nachweis am Feld',
  Array.isArray(matrixField?.evidences) &&
    matrixField.evidences.some((e) => e.evidence.id === evId),
);
...
const badType = await req(
  'POST',
  `/evidence/${evId}/upload-url`,
  { fileName: 'schad.exe', contentType: 'application/octet-stream', sizeBytes: 1000 },
  student,
);
...
const presign = await req(
  'POST',
  `/evidence/${evId}/upload-url`,
  { fileName: 'nachweis.pdf', contentType: 'application/pdf', sizeBytes: 1024 },
  student,
);
check(
  'Upload gültig → presigned URL',
  presign.status === 201 && presign.body?.uploadUrl?.startsWith('http'),
);
const put = await fetch(presign.body.uploadUrl, {
  method: 'PUT',
  headers: { 'Content-Type': 'application/pdf' },
  body: Buffer.from('%PDF-1.4 x'),
});
check('Datei-PUT an MinIO → 200', put.status === 200);
const confirm = await req(
  'POST',
  `/evidence/${evId}/upload-confirm`,
  { key: presign.body.key, fileName: 'nachweis.pdf' },
  student,
);
...
await req(
  'POST',
  `/submissions/${confirm.body.submissionId}/reject`,
  { reason: 'Bitte ergänzen.' },
  teacher,
);
const linkSub = await req(
  'POST',
...
await req(
  'POST',
  `/submissions/${linkSub.body.submissionId}/reject`,
  { reason: 'Link defekt.' },
  teacher,
);
const textSub = await req(
  'POST',
  `/evidence/${evId}/submissions`,
  { text: 'Meine Lösung als Text.' },
  student,
);
...
const fieldEvs =
  reordered.body?.matrix?.bands?.[0]?.fields?.find((f) => f.id === fieldId)?.evidences ?? [];
check('Matrix-Reihenfolge getauscht', fieldEvs[0]?.evidence?.id === evId2);

// ── Bild-Upload (Rich-Text-Asset) ─────────────────────────────────
const imgBad = await req(
  'POST',
  '/assets/image-upload-url',
  { fileName: 'x.exe', contentType: 'x' },
  teacher,
);
...
const img = await req(
  'POST',
  '/assets/image-upload-url',
  { fileName: 'foto.png', contentType: 'image/png', sizeBytes: 2048 },
  teacher,
);
check(
  'Bild-Upload-URL → presigned + publicUrl',
  img.status === 201 && img.body?.uploadUrl?.startsWith('http') && !!img.body?.publicUrl,
);
const imgPut = await fetch(img.body.uploadUrl, {
  method: 'PUT',
...
const studentImgPost = await req(
  'POST',
  '/assets/image-upload-url',
...
check(
  'Nachweis mit allowScreenshot/allowPaste',
  ev3.body?.config?.allowScreenshot === true && ev3.body?.config?.allowPaste === true,
);
check('Nachweis mit allowExpertTalk', ev3.body?.config?.allowExpertTalk === true);
const multi = await req(
  'POST',
  `/evidence/${evId3}/submit`,
  { text: 'Meine Begründung', link: 'https://example.com/x' },
  student,
);
...
const evTalk = await req(
  'POST',
  '/evidence',
  {
    moduleId,
    title: { de: 'Fachgespräch OS' },
    isVisible: true,
    fieldIds: [fieldId],
...
check(
  'Reines Fachgespräch ohne Inhalt → submitted',
  talkSubmit.body?.status === 'SUBMITTED',
  `status=${talkSubmit.status}`,
);

// Gegenprobe: ohne Fachgespräch ist leere Einreichung weiterhin 400
const evPlain = await req(
  'POST',
  '/evidence',
  {
    moduleId,
...
const att = await req(
  'POST',
  '/assets/attachment-upload-url',
  { fileName: 'vorlage.pdf', contentType: 'application/pdf' },
  teacher,
);
check('Anhang-Upload-URL → presigned', att.status === 201 && !!att.body?.key);
await fetch(att.body.uploadUrl, {
  method: 'PUT',
  headers: { 'Content-Type': 'application/pdf' },
  body: Buffer.from('%PDF vorlage'),
});
await req(
  'PATCH',
  `/evidence/${evId3}`,
  { config: { allowScreenshot: true, attachmentKey: att.body.key, attachmentName: 'vorlage.pdf' } },
  teacher,
);
const sView3 = await req('GET', `/evidence/student/${evId3}`, null, student);
check(
  'Lernende:r erhält Anhang-Download-URL',
  typeof sView3.body?.attachmentUrl === 'string' && sView3.body.attachmentUrl.startsWith('http'),
);
