...
// ── Setup ─────────────────────────────────────────────────────────
const t = await req('POST', '/auth/dev-login', { email: `lp-teacher-${Date.now()}@demo.ch`, role: 'TEACHER' });
const teacher = t.body?.token;
const s = await req('POST', '/auth/dev-login', { email: `lp-student-${Date.now()}@demo.ch`, role: 'LEARNER' });
const student = s.body?.token;
check('Logins', !!teacher && !!student);

const modNum = `LP${Date.now()}`;
const mod = await req('POST', '/modules', { number: modNum, title: { de: 'Lernpfad-Test' } }, teacher);
...
// ── Pfad anlegen ──────────────────────────────────────────────────
const created = await req('POST', `/matrices/${matrixId}/paths`, { name: 'Empfohlen', fieldIds: [fA, fB, fA2], isActive: true }, teacher);
check('Pfad erstellen → 201', created.status === 201, `status=${created.status}`);
const pathId = created.body?.id;
check('Pfad hat 3 Schritte in Reihenfolge', created.body?.steps?.length === 3 && created.body.steps[0].fieldId === fA);
check('Pfad ist aktiv', created.body?.isActive === true);

// Feld fremder Matrix → 400
const bad = await req('POST', `/matrices/${matrixId}/paths`, { name: 'X', fieldIds: ['00000000-0000-0000-0000-000000000000'] }, teacher);
check('Unbekanntes Feld → 400', bad.status === 400);

// Leerer Name → 400
const noName = await req('POST', `/matrices/${matrixId}/paths`, { name: '  ', fieldIds: [fA] }, teacher);
...
const second = await req('POST', `/matrices/${matrixId}/paths`, { name: 'Alternativ', fieldIds: [fB, fA] }, teacher);
...
check('Reihenfolge aktualisiert (2 Schritte)', upd.body?.steps?.length === 2 && upd.body.steps[0].fieldId === fB);
...
check('Alle Schritte zunächst OPEN', view.body?.path?.steps?.every((s) => s.status === 'OPEN'));
...
const ev = await req('POST', '/evidence', { moduleId, title: { de: 'Nachweis A1B' }, isVisible: true, fieldIds: [fA] }, teacher);
