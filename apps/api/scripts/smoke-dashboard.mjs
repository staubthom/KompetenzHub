...
const mod = await req(
  'POST',
  '/modules',
  { number: modNum, title: { de: 'Dashboard-Test' } },
  teacher,
);
...
await req(
  'POST',
  `/submissions/${sub1.body.submissionId}/evaluation`,
  { points: 8, level: 'INTERMEDIATE' },
  teacher,
);
...
check(
  'Student1 Feld GRADED + Punkte',
  rowS1?.cells?.[fieldId]?.status === 'GRADED' && Number(rowS1?.cells?.[fieldId]?.points) === 8,
);
...
const queue = await req(
  'GET',
  `/submissions?status=SUBMITTED&classId=${cls.body.id}`,
  null,
  teacher,
);
check(
  'Queue (classId-Filter) → nur submitted dieser Klasse',
  Array.isArray(queue.body) &&
    queue.body.length === 1 &&
    queue.body[0].enrollment.class.id === cls.body.id,
);
