...
const s1 = await req('POST', '/auth/dev-login', {
  email: `et-stud-a-${Date.now()}@demo.ch`,
  role: 'LEARNER',
});
const studentA = s1.body?.token;
const s2 = await req('POST', '/auth/dev-login', {
  email: `et-stud-b-${Date.now()}@demo.ch`,
  role: 'LEARNER',
});
...
check(
  'Start ohne/mit KI liefert 409 oder 201',
  gate.status === 409 || gate.status === 201,
  `status=${gate.status}`,
);
...
check(
  'GET /expert-talk/available → available true',
  avail.status === 200 && avail.body?.available === true,
);
...
const created = await req(
  ...
  {
    topic: 'Betriebssysteme konfigurieren',
    context: '<p>Beschreibe die <strong>UAC</strong>-Konfiguration.</p>',
  },
  studentA,
);
...
check(
  'KI stellt erste Frage (assistant)',
  Array.isArray(created.body?.messages) &&
    created.body.messages.length === 1 &&
    created.body.messages[0].role === 'assistant' &&
...
const msg = await req(
  'POST',
  `/expert-talk/sessions/${sessionId}/messages`,
  { content: 'UAC fragt bei Adminrechten nach.' },
  studentA,
);
check(
  'Antwort senden → 200 (assistant)',
  msg.status === 200 && msg.body?.role === 'assistant' && msg.body?.content.length > 0,
);

// Leere Antwort → 422
const emptyMsg = await req(
  'POST',
  `/expert-talk/sessions/${sessionId}/messages`,
  { content: '' },
  studentA,
...
check(
  'Reihenfolge assistant → user → assistant',
...
check(
  'Liste enthält Gespräch',
  Array.isArray(list.body) && list.body.some((x) => x.id === sessionId),
);
...
const mod = await req(
  'POST',
  '/modules',
  { number: modNum, title: { de: 'KI-Modul-Üben' } },
  teacher,
);
...
check(
  'Modul-Session KI stellt erste Frage',
  modSession.body?.messages?.[0]?.role === 'assistant' &&
    modSession.body.messages[0].content.length > 0,
);
const modList = await req('GET', '/expert-talk/sessions', null, studentA);
check(
  'Liste enthält Modul-Session mit mode',
  modList.body.some((x) => x.id === modSession.body.id && x.mode === 'module'),
);

// Modul ohne Kompetenzen → 422
const emptyMod = await req(
  'POST',
  '/modules',
  { number: `ETX${Date.now()}`, title: { de: 'Leer' } },
  teacher,
);
const clsE = await req(
  'POST',
  '/classes',
  { name: 'ET-Leer', moduleId: emptyMod.body.id },
  teacher,
);
...
const noComp = await req(
  'POST',
  '/expert-talk/module-sessions',
  { moduleId: emptyMod.body.id },
  studentA,
);
check('Modul ohne Kompetenzen → 422', noComp.status === 422, `status=${noComp.status}`);

// ── KI-Quelle: eigene KI vs. freigegebene Lehrer-KI ───────────────
const sc = await req('POST', '/auth/dev-login', {
  email: `et-stud-c-${Date.now()}@demo.ch`,
  role: 'LEARNER',
});
