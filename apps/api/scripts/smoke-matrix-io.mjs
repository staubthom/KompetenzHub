...
const t = await req('POST', '/auth/dev-login', {
  email: `mio-teacher-${Date.now()}@demo.ch`,
  role: 'TEACHER',
});
const teacher = t.body?.token;
const s = await req('POST', '/auth/dev-login', {
  email: `mio-student-${Date.now()}@demo.ch`,
  role: 'LEARNER',
});
...
const mod = await req(
  'POST',
  '/modules',
  { number: modNum, title: { de: 'Export-Test' } },
  teacher,
);
...
await fetch(att.body.uploadUrl, {
  ...
  body: Buffer.from('%PDF-1.4 Testdokument'),
});
...
await req(
  ...
  { name: 'Pfad 1', fieldIds: [fA, fB], isActive: true },
  teacher,
);
check(
  'Setup vollständig',
  !!matrixId && !!fA && !!fB && !!ev.body?.id && !!attachmentKey && !!imgUrl,
);
...
check(
  'Manifest listet 2 Assets (Bild + Anhang)',
  Array.isArray(manifest.assets) && manifest.assets.length === 2,
);
...
check(
  'Instruktion verweist auf zip-Pfad (assets/…) statt URL',
  !!evExp && evExp.instructions.de.includes('assets/') && !evExp.instructions.de.includes('http'),
);
check(
  'Anhang als zip-Pfad referenziert',
  !!evExp?.attachment?.path && evExp.attachment.name === 'vorlage.pdf',
);
check(
  'Export ohne personenbezogene Daten',
  JSON.stringify(manifest).includes('submission') === false,
);

// ── Import (Round-Trip, Original existiert noch) ──────────────────
const imp = await importZip(exp.buffer, teacher);
check(
  'Import → 201 mit neuem Modul',
  imp.status === 201 && !!imp.body?.moduleId,
  `status=${imp.status}`,
);
check(
  'Neue Modulnummer (Original existiert)',
  imp.body?.number === `${modNum}-2`,
  `number=${imp.body?.number}`,
);
const impDetail = await req('GET', `/modules/${imp.body.moduleId}`, null, teacher);
check(
  'Importierter Titel enthält „(Importiert)"',
  (impDetail.body?.title?.de ?? '').includes('(Importiert)'),
);
...
check(
  'Import: Bild-URL neu gesetzt (rte/, kein assets/)',
  !!impEv &&
    impEv.instructions.de.includes('/rte/') &&
    !impEv.instructions.de.includes('assets/') &&
    impEv.instructions.de.includes('http'),
);
check(
  'Import: neuer Anhang-Key (attachments/, != Original)',
  !!impEv?.config?.attachmentKey &&
    impEv.config.attachmentKey.includes('attachments/') &&
    impEv.config.attachmentKey !== attachmentKey,
);
...
badVersionZip.addFile(
  'matrix.json',
  Buffer.from(
    JSON.stringify({
      schemaVersion: 99,
      kind: 'matrix-export',
      module: { number: 'x', title: { de: 'x' } },
      bands: [],
    }),
  ),
);
